...
Android/Kotlin 应用，基于 AccessibilityService 实现自动化打卡。调度手势序列，检测目标应用弹窗事件，记录历史，可选邮件通知。
...
├── AutoClockService.kt      # 无障碍服务，手势序列和弹窗事件检测
...
**数据流**：`MainActivity` 保存配置 → `AlarmScheduler` 调度闹钟 → `AlarmReceiver` 触发 → `AutoClockService` 执行手势 → `ClockSuccessDetector` 检测弹窗事件 → 3秒后返回桌面 → 杀死目标应用 → 打开向日葵 → 可选邮件通知。
