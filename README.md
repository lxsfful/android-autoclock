...
- 精确闹钟调度，支持两个可配置的时间窗口
- 分钟+秒数完全随机化，避免规律性
- 仅在云之家范围内读取窗口文本/内容描述，用于检测快捷打卡响应
- 本地运行历史记录存储
...
**数据流**：`MainActivity` 保存配置 → `AlarmScheduler` 随机精确闹钟调度 → `AlarmReceiver` 触发 → `AutoClockService` 唤醒、回桌面并点击桌面“云之家快捷打卡” → 等待快捷打卡界面并执行界面内“打卡”保险点击 → `ClockSuccessDetector` 在云之家窗口内检测响应 → 3秒后返回桌面 → 请求结束云之家后台进程 → 通过包名打开向日葵（失败时坐标兜底） → 可选邮件通知。
...
- 监听云之家包名的窗口变化事件，并读取目标窗口文本/内容描述来检测快捷打卡响应
